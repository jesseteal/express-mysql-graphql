...
let pool = null;
let connection_config = { connectionLimit: 10 };
...
/*
  where examples:
    ...
    { userId: null }
 */
const where_args = (where, table, token, options, subgraph) => {
    const wheres = [];
...
const first = async (...args) => {
    const [rows] = await (0, exports.query)(...args);
    return rows[0];
};
exports.first = first;
const db = {
...
const generate_schema = async (options) => {
    ...
    const [tables] = await conn.query(constants_1.default.SQL_SCHEMA(connection_config.database));
    const [relationships] = await conn.query(constants_1.default.SQL_RELATIONSHIPS(connection_config.database));
    const inputs = tables
        .map((r) => {
    ...
    ` +
        tables
            .map((t) => `${t.TABLE_NAME}(limit: Int, offset: Int, where: String, order: String): [${t.TABLE_NAME}]`)
            .join("\n") +
        `
  }`;
    const mutations = `
    ...
    tables.forEach((t) => {
        resolvers.Query[t.TABLE_NAME] = async (obj, args, { req }) => {
            ...
            const { wheres, params, fields } = where_args(where, t.TABLE_NAME, req?.auth, options);
        ...
        resolvers.Mutation[createName] = async (obj, args, { req }) => {
            let { input } = args;
            const columns = [];
            const values = [];
            // BEFORE INSERT
            ...
            let id = await conn
                .query(`INSERT IGNORE INTO ${t.TABLE_NAME} (\`${columns.join("`,`")}\`) values (${columns.map((c) => "?").join(",")})`, values)
                .then((r) => r[0].insertId)
                .catch(db.dlog);
        ...
        resolvers.Mutation[deleteName] = async (obj, { input }, { req }) => {
            const keys = t.PKEYS.split(",");
            const columns = [];
            const values = [];
        ...
        resolvers.Mutation[updateName] = async (obj, { input }, { req }) => {
            const keys = t.PKEYS.split(",");
            let keys_found = true;
            keys.forEach((key, i) => {
    ...
    for (var t of relationships) {
      ...
      `;
        resolvers[TABLE_NAME][`${FROM_COL}_${LINKED_TABLE}`] = async (parent, args, req) => {
            const { where } = args;
            const { wheres, params } = where_args(where, LINKED_TABLE, req.user, options, true // subgraph
            );
    ...
    for (var t of relationships) {
        ...
        if (FROM_COL === TO_COL || TO_COL === "id") {
            resolvers[LINKED_TABLE][TABLE_NAME] = async (parent, args, req) => {
                const { limit, offset, where, order } = args;
                const { wheres, params } = where_args(where, TABLE_NAME, req.user, options, true);
